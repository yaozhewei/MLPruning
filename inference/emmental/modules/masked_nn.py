...
# This function is from https://stackoverflow.com/questions/16873441/form-a-big-2d-array-from-multiple-smaller-2d-arrays
...
class MaskedLinear(nn.Linear):
    ...
    def __init__(
        ...
        # this is used for final block-wise pruning, for init we do not need to worry about that!
        ...
        if self.row_pruning:
            self.mask_scores = nn.Parameter(torch.Tensor(self.weight.size(0), 1)) # number of rows * 1
            self.init_mask(self.mask_scores)
            self.threshold_row = nn.Parameter(torch.zeros(1)+10.0)
        if self.head_pruning:
            self.head_mask_scores = nn.Parameter(torch.Tensor(self.head_split, 1)) # number of heads * 1
    ...
    def get_mask(self):
        # get head mask
        if self.head_pruning:
            mask_head = TopKBinarizer.apply(self.head_mask_scores, self.threshold_head, -1) # for now, only support this
        else:
            mask_head = None

        if self.row_pruning:
            mask = TopKBinarizer.apply(self.mask_scores, self.threshold_row, -1)
        else:
            mask = None
    ...
    def make_inference_pruning(self, blocksize):
        ...
        if not self.head_pruning:
            mask_head = torch.ones_like(self.weight[:, 0]).type('torch.BoolTensor').view(-1)
        else:
            mask_head = mask_head.type('torch.BoolTensor').view(-1)
            mask_head = torch.repeat_interleave(mask_head, weight_shape[0]//self.head_split)
    ...
    def enable_block_pruning(self, block_size):
        ...
        self.block_mask_scores = nn.Parameter(torch.Tensor(mask_size_row*mask_size_col, 1, 1)) # number of row_block * col_block
    ...
    def get_block_wise_pruning(self):
        ...
        mask_block = TopKBinarizer.apply(self.block_mask_scores, self.threshold_block, -1) # for now, only support this        
        return mask_block
    ...
    def make_block_wise_inference_pruning(self):
        ...
        if self.runsparse:
            return self._make_block_wise_inference_pruning_sparse(real_param_count)
        else:
    ...
    def _make_block_wise_inference_pruning_sparse(self, real_param_count):
        ...
        if mask_block.sum()/mask_size_row/mask_size_col < threshold:
            mask_block = mask_block.reshape((mask_size_row, mask_size_col)).type(torch.LongTensor)
            mask_block = torch.transpose(mask_block, 0, 1).reshape((1, mask_size_col, mask_size_row))
            self.op = triton.ops.blocksparse.matmul(mask_block, block, "dds", trans_a=False, trans_b=False)
            tmp_weight = torch.transpose(self.weight, 0, 1).reshape((1, 1, self.weight.shape[1], self.weight.shape[0]))
            self.sparse_weight = triton.testing.sparsify_tensor(tmp_weight, mask_block, block)
            ...
            tmp_weight = blockshaped(self.weight, self.block_rows, self.block_cols) # n-block x 32 x 32
