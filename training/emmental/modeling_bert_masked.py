...
# We add more functionalities as well as remove unnecessary functionalitiesimport logging
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # If head pruning, we need to make the masking_score for qkv to be the same
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, res_mask=None):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        if res_mask is not None:
            input_tensor[:, :, res_mask] = input_tensor[:, :, res_mask] + hidden_states
        else:
...
class BertAttention(nn.Module):
    ...
    def make_structural_pruning(self, blocksize):
        ...
        output_mask = self.output.dense.make_inference_pruning(blocksize) # we need this to do the residual connection
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, res_mask=self.output_mask)
...
class BertLayer(nn.Module):
    ...
    def make_structural_pruning(self, blocksize):
        ...
        intermediate_mask = self.intermediate.dense.make_inference_pruning(blocksize) # we need this to do the residual connection
    ...
    def forward(
        ...
        layer_output = self.output(intermediate_output, attention_output, res_mask=self.output_mask)
